...
export const HowItWorks: React.FC = () => {
  ...
  return (
    <section className="py-16 sm:py-24 bg-brand-slate-50 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-1/2 left-12 right-12 h-0.5 bg-brand-slate-200 -translate-y-12 z-0"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
            {steps.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-white rounded-3xl p-7 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1.5 border border-brand-slate-200/80 flex flex-col justify-between"
                >
                  <div>
                    {/* Step badge & Icon */}
                    <div className="flex items-center justify-between mb-6">
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-brand-navy to-brand-teal text-white flex items-center justify-center shadow-md">
                        <Icon className="w-6 h-6 text-white" />
                      </div>
                      <span className="text-2xl font-black font-display text-brand-slate-300">
                        {item.step}
                      </span>
                    </div>
        ...
        <div className="mt-12 text-center">
          <button
            ...
            className="inline-flex items-center gap-2 bg-gradient-to-r from-brand-accent to-brand-accent-hover text-brand-navy font-bold text-sm px-8 py-3.5 rounded-2xl shadow-glow-accent hover:shadow-xl transition transform active:scale-98"
          >
